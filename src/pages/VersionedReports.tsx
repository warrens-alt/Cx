...
export default function VersionedReports() {
  ...
  return <div className="cx-page cx-report">
    ...
    <form id="report-scope" className="enterprise-card cx-report-form" aria-describedby={currentRelease && scopeError ? "report-preflight" : undefined} onSubmit={e=>{e.preventDefault();submit();}}>
      ...
      <footer className="cx-report-form-footer"><p><Info size={14} aria-hidden="true"/>Source completeness is separate from calculation checks.</p><button type="submit" disabled={!currentRelease || !!catalogue.error || catalogue.isFetching || !!scopeError || report.isFetching} className="cx-button-primary">{report.isFetching?<RefreshCw size={15} className="animate-spin"/>:<ArrowRight size={15}/>}Create snapshot-bound report</button></footer>
    </form>
    ...
    {data && <section className="cx-report-results" aria-label="Report results">
      ...
      <ReportBreakdown key={data.executionId} rows={data.groups} grouping={data.request.grouping} busy={busy} formatValue={metricValue} inspect={inspect}/>
