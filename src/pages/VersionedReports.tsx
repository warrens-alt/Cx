...
export default function VersionedReports() {
  ...
  return <div className="max-w-[1500px] mx-auto p-4 sm:p-8 space-y-6">
    <header><p className="text-xs uppercase tracking-wide">Versioned analytical reporting</p><h1 className="text-2xl font-bold mt-1">{PAGE_TITLES['/reports']}</h1><p className="mt-2 text-sm">Snapshot-bound metrics, exact decimal values and traceable records. UTC dates are used explicitly; source completeness is assessed separately from calculation checks.</p></header>
    ...
    {catalogue.data?.release && <div className="enterprise-card p-4 text-sm"><strong>Available release: {catalogue.data.release.releaseId}</strong><p>Data Available Through: {catalogue.data.release.cutoff}. {catalogue.data.release.sourceBatchIds.length} retained source batches.</p></div>}
    <form className="enterprise-card p-5 space-y-4" onSubmit={e=>{e.preventDefault();if(!catalogue.data?.available)return;setSubmitted({request:scope,releaseId:catalogue.data.release.releaseId});}}>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <label className="text-sm">Date basis<select aria-label="Date basis" value={basis} onChange={e=>setBasis(e.target.value as any)} className="block border rounded p-2 w-full"><option value="capture_cohort">Lead Capture Cohort</option><option value="event_date">Event Date</option></select></label>
        ...
        <label className="text-sm">Currency<input required aria-label="Currency" pattern="[A-Z]{3}" value={currency} onChange={e=>setCurrency(e.target.value)} className="block border rounded p-2 w-full" /></label>
      </div>
