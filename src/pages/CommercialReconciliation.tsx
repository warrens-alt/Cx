...
export default function CommercialReconciliation() {
  ...
  const [sort,setSort]=useState('collected_value'), [selectedVendor,setSelectedVendor]=useState<string|null>(null), [inspection,setInspection]=useState<EvidenceSelection|null>(null);
  ...
  return <div className="cx-page cx-ops-page cx-commercial-page">
    ...
    {report&&<>
      ...
      <section id="commercial-ledger" className="enterprise-card cx-ops-table-card"><header><div><p className="cx-ops-eyebrow">Vendor reconciliation</p><h2>Commercial stage table</h2><p>Exact values grouped by vendor. Selecting a value opens records from the same release and execution.</p></div><button type="button" className="cx-button-secondary" onClick={()=>exportRows(filtered.map(row=>({vendor:row.rawGroup,...Object.fromEntries(STAGES.map(id=>[id,row.metrics[id]?.value??null]))})),`cx-commercial-${workspace.request.startDate}-${workspace.request.endDate}.json`)}><Download size={15}/>Export {filtered.length} matching rows</button></header>
