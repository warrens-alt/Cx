...
import { ShieldCheck, Layers3, Palette, ArrowUpRight, Download, Filter, Clock3 } from 'lucide-react';
...
export default function Vetting(){
  ...
  return <PageShell><div className="vetting-page">
    <header className="vetting-hero"><div><span className="vetting-eyebrow"><ShieldCheck size={15}/>LEAD CLASSIFICATION INTELLIGENCE</span><h1>Vetting</h1><p>Understand your class leads, colour leads and the outcomes recorded against them.</p><div className="vetting-hero-meta"><span><Layers3 size={14}/>Separate class and colour axes</span><span><Clock3 size={14}/>{startDate} to {endDate} · capture cohorts</span></div></div><button type="button" className="cx-button-secondary" disabled={!show} onClick={exportAll}><Download size={15}/>Export complete analysis</button></header>
    ...
    {!show?<DataState loading={result.loading||result.fetching} error={result.error} empty={!data} retry={result.refetch}/>:<div role="tabpanel" id={`vetting-panel-${tab}`} aria-labelledby={`vetting-tab-${tab}`} className="vetting-panel" key={`${selectedClient}:${data.scope.startDate}:${data.scope.endDate}:${classValue}:${colourValue}`}>
      ...
      {tab==='overview'&&<>
        <div className="vetting-kpis">{([['leads','Included Leads'],['classRecorded','Class Result Recorded'],['namedColour','Recognised Colour'],['bothRecorded','Class + Named Colour']] as [VettingMetric,string][]).map(([key,label])=>{
          const change=periodChange(data.current[key],data.previous[key]);return <article key={key} className="enterprise-card"><span>{label}</span><strong>{exactLabel(data.current[key])}</strong><small>{key==='leads'?`${data.scope.days} capture days`: `${pct(countRatio(data.current[key],data.current.leads))} of included leads`}</small><p><ArrowUpRight size={13}/>{change.percent===null?'No comparable prior denominator':`${change.percent}% vs previous window`}</p></article>;
        })}</div>
