...
export function useAnalyticsData<T = any>(endpoint: string, extraParams: Record<string, any> = {}): FetchAnalyticsResult<T> {
  ...
  return { data: result.data?.data ?? null, metadata: result.data?.metadata ?? null, loading: result.isLoading,
    error: result.error instanceof Error ? result.error.message : null, refetch: () => { void result.refetch(); } };
}
