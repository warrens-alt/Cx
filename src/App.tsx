    ...
    <div className="cx-workarea">
      <header className="cx-topbar">
        ...
        <div className="cx-topbar-actions"><button type="button" className="cx-search-trigger" aria-label="Search pages" onClick={openSearch}><Search size={16}/><span>Find a page</span><kbd>⌘ K</kbd></button>
          ...
          <label className="cx-workspace-select"><span>Workspace</span><select aria-label="Active workspace" value={selectedClient} onChange={event=>setSelectedClient(event.target.value)} disabled={clientLoading || !clients.length}>{!selectedClient&&<option value="" disabled>Choose an authorised workspace</option>}{clients.map(client=><option key={client.id} value={client.id}>{client.name}</option>)}</select></label>
        </div>
      </header>
