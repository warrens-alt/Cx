...
export default function VettingChart({title,description,rows,series,unit='leads',ordered=false,disjoint=false,initial='bar',onSelect}:Props){
  ...
  const x=<XAxis dataKey="__name" tick={{fontSize:12}} tickFormatter={v=>String(v).length>19?String(v).slice(0,17)+'…':String(v)} minTickGap={18}/>;
  const y=<YAxis tickFormatter={axis} width={60} tick={{fontSize:12}} domain={[0,'auto']}/>;
  ...
  else if(effective==='line')chart=<LineChart data={points} margin={{top:16,right:20,bottom:16,left:0}} accessibilityLayer>{grid}{x}{y}{tooltip}{legend}{series.map((s,i)=><Line key={s.key} dataKey={`__v${i}`} name={s.label} stroke={s.colour||categoryColour(s.label,s.key)} strokeWidth={2.5} type="linear" connectNulls={false} dot={points.length<35?{r:3}:false} isAnimationActive={false}/>)}</LineChart>;
  else if(effective==='area')chart=<AreaChart data={points} margin={{top:16,right:20,bottom:16,left:0}} accessibilityLayer>{grid}{x}{y}{tooltip}{legend}{series.map((s,i)=><Area key={s.key} dataKey={`__v${i}`} name={s.label} stroke={s.colour||categoryColour(s.label,s.key)} fill={s.colour||categoryColour(s.label,s.key)} fillOpacity={.1} type="linear" connectNulls={false} isAnimationActive={false}/>)}</AreaChart>;
  else if(effective==='column')chart=<BarChart data={points} margin={{top:16,right:20,bottom:16,left:0}} accessibilityLayer>{grid}{x}{y}{tooltip}{legend}{series.map((s,i)=><Bar key={s.key} dataKey={`__v${i}`} name={s.label} fill={s.colour||categoryColour(s.label,s.key)} maxBarSize={40} isAnimationActive={false} onClick={click}/>)}</BarChart>;
  else chart=<BarChart data={points} layout="vertical" margin={{top:8,right:20,bottom:16,left:0}} accessibilityLayer><CartesianGrid horizontal={false} strokeDasharray="3 4" stroke="#dce5ec"/><XAxis type="number" tickFormatter={axis} tick={{fontSize:12}}/><YAxis dataKey="__name" type="category" width={122} tick={{fontSize:12}} tickFormatter={v=>String(v).length>19?String(v).slice(0,17)+'…':String(v)} interval={0}/>{tooltip}{legend}{series.map((s,i)=><Bar key={s.key} dataKey={`__v${i}`} name={s.label} fill={s.colour||categoryColour(s.label,s.key)} maxBarSize={23} isAnimationActive={false} onClick={click}>{disjoint&&series.length===1&&points.map((p,j)=><Cell key={p.__name} fill={colourFill(p.__name,j)}/>)}</Bar>)}</BarChart>;
