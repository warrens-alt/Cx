...
export function comparison(raw: number | null, semantic: number | null): { status: CheckStatus; difference: number | null; discrepancy: string } {
  if (raw === null || semantic === null || !Number.isFinite(raw) || !Number.isFinite(semantic)) return { status: 'NOT_VERIFIED', difference: null, discrepancy: 'Independent measurements are unavailable.' };
  const difference = semantic - raw;
  return difference === 0 ? { status: 'PASS', difference, discrepancy: 'Independent measurements match.' } : { status: 'FAIL', difference, discrepancy: `Measured difference: ${difference}.` };
}
