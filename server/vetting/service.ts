import { COUNT_KEYS, VETTING_SECTIONS, VETTING_VERSION, type VettingCounts, type VettingGroup, type VettingReport } from '../../contracts/vetting';
...
function counts(raw: any): VettingCounts {
  ...
  const result = Object.fromEntries(COUNT_KEYS.map(k=>[k,count(raw[k],k!=='leads')])) as VettingCounts;
  result.classMeanSeconds=decimal(raw.classMeanSeconds);result.colourMeanSeconds=decimal(raw.colourMeanSeconds);
...
export function validateVettingResult(raw: any) {
  ...
  for(const [index,period]of ['current','previous'].entries()){
    const d=raw.diagnostics[index];if(d.period!==period)throw new RequestError('Invalid diagnostic period',502);
  ...
  return {current,previous,groups,diagnostics:raw.diagnostics as VettingReport['diagnostics'],timing};
}
export async function getVettingReport(input: VettingInput, access?: SourceAccess): Promise<VettingReport> {
  ...
  return {...result,fields:compiled.fields,scope:{...scope,clientId:client.id},evidence:{version:VETTING_VERSION,table,jobId:execution.jobId,referencedTables:execution.referencedTables,bytesProcessed:execution.bytesProcessed,
