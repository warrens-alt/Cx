...
export function populationQuery(req: ReportRequest, release: ReleaseManifest): CompiledQuery {
  ...
  population AS (
    SELECT 'fetched_leads' AS metric_id,l.entity_key,l.entity_key AS lead_key,CAST(NULL AS STRING) AS vendor,l.source,l.medium,l.captured_at,l.captured_at AS event_at,CAST(1 AS NUMERIC) AS numerator,CAST(NULL AS NUMERIC) AS denominator,l.source_record_id,l.batch_id
      FROM l ${req.filters.vendor ? 'WHERE EXISTS(SELECT 1 FROM d0 WHERE d0.lead_key=l.entity_key)' : ''}
    UNION ALL SELECT 'delivered_episodes',entity_key,lead_key,vendor,source,medium,captured_at,delivered_at,1,NULL,source_record_id,batch_id FROM d
    UNION ALL SELECT 'call_attempts',entity_key,lead_key,vendor,source,medium,captured_at,event_at,1,NULL,source_record_id,batch_id FROM c
    UNION ALL SELECT 'called_episodes',d.entity_key,d.lead_key,d.vendor,d.source,d.medium,d.captured_at,called.first_call,1,NULL,d.source_record_id,d.batch_id FROM d JOIN called USING(entity_key)
    UNION ALL SELECT 'call_coverage',d.entity_key,d.lead_key,d.vendor,d.source,d.medium,d.captured_at,d.delivered_at,IF(called.entity_key IS NULL,0,1),1,d.source_record_id,d.batch_id FROM d LEFT JOIN called USING(entity_key)
    UNION ALL SELECT 'sale_events',entity_key,lead_key,vendor,source,medium,captured_at,event_at,1,NULL,source_record_id,batch_id FROM s
    UNION ALL SELECT 'activation_events',entity_key,lead_key,vendor,source,medium,captured_at,event_at,1,NULL,source_record_id,batch_id FROM a
    UNION ALL SELECT 'sale_activation_rate',s.entity_key,s.lead_key,s.vendor,s.source,s.medium,s.captured_at,s.event_at,IF(activated.sale_key IS NULL,0,1),1,s.source_record_id,s.batch_id FROM s LEFT JOIN activated ON activated.sale_key=s.entity_key
    UNION ALL SELECT CONCAT(stage,'_value'),entity_key,lead_key,vendor,source,medium,captured_at,event_at,amount_delta,NULL,source_record_id,batch_id FROM f
...
export function compileEvidence(req: ReportRequest, release: ReleaseManifest, metric: string, group: string | null): CompiledQuery {
  ...
  return { query: `${query} SELECT metric_id,entity_key,lead_key,vendor,source,medium,CAST(captured_at AS STRING) AS captured_at,CAST(event_at AS STRING) AS event_at,
      CAST(numerator AS STRING) AS numerator,CAST(denominator AS STRING) AS denominator,source_record_id,batch_id
