...
export async function verifyFrontendOptimisations(browser, base) {
  ...
  for (const viewport of [{ width: 1440, height: 1000 }, { width: 390, height: 844 }]) {
    ...
    try {
      ...
      await filters.getByRole('option', { name: 'Vendor A', exact: true }).waitFor({ state: 'attached' });
      ...
      await page.getByText(/Observation cutoff is later than the available release/).waitFor();
