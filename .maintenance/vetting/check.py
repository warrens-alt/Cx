...
# The application feature needs no workflow modification. Keep main's existing CI unchanged.
subprocess.run(['git','restore','--source='+base,'--staged','--worktree','--','.github/workflows/ci.yml'],check=True)
manifest.pop('.github/workflows/ci.yml')
errors=[]
...
print(f'All {len(manifest)} application files match their reviewed source hashes. Transfer files are excluded; existing main CI is unchanged.')
